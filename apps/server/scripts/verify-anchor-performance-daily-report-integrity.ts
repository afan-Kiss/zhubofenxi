...
/**
 * 2026-07-04 真实发货黄金值（2026-07-07 更新；2026-07-30 口径说明）
 *
 * 旧黄金值全店 8 单（子杰 3 / 小艺 2 / 飞云 3）包含后来进入售后/关闭的订单：
 * - P798690281340293991（子杰，售后处理中：待商家收货）→ 仍剔除
 * - P798714646273341211（飞云，售后完成）→ 仍剔除
 * - P798682298893211811（关闭/退款类）→ 仍剔除
 *
 * 「售后已取消/关闭无退款且已发货」现可计入真实发货；上列剔除单不属于该例外。
 * 当前日报「真实发货」黄金值仍为 5 单（与库内这三单状态一致时）。
 */

...
/** 经营看板 StatCard：支持 ReactNode 标签（含口径 Tooltip） */
export const BoardStatCard: React.FC<BoardStatCardProps> = ({
  label,
  value,
  accent = 'primary',
  onClick,
  ...
  return (
    <div
      ...
      className={`group flex min-h-[96px] min-w-0 flex-col justify-between rounded-2xl border border-white/70 bg-[var(--color-card)] px-4 py-3 shadow-[0_8px_24px_rgba(15,23,42,0.06)] transition ${
        clickable
          ? 'card-clickable cursor-pointer hover:-translate-y-0.5 hover:border-rose-200 hover:shadow-lg'
          : ''
      }`}
    >

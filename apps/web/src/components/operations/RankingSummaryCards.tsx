...
interface Props {
  ...
  getDrillRequest?: (item: BossSummaryItem) => OperationsBiDrillRequest | null
}

export const RankingSummaryCards: React.FC<Props> = ({ items, getDrillRequest }) => {
  if (items.length === 0) {
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {items.map((item) => {
        const drillRequest = getDrillRequest?.(item) ?? null
        return (
          <div
            key={item.title}
          ...
          >
            ...
            {drillRequest ? (
              <div className="mt-2">
                <OperationsBiDrillLinkButton request={drillRequest} label="看数据来源" />
              </div>
            ) : null}
          </div>
        )

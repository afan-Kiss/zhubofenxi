...
export const METRIC_EXPLAIN: Record<BoardMetricExplainKey, string> = {
  ...
  totalGmv:
    ...
    '支付金额：统计所选日期范围内有支付时间的已支付订单实付金额汇总。已退款订单仍计入，退款金额与退款单数另行统计。',

  validSalesAmount:

    '有效成交额：与运营报表同一口径，仅统计已完成/已签收、无在途售后且未成功退款的订单成交金额。',

  actualSignedAmount:

    '已签收金额：统计所选日期范围内已签收/已完成、且符合实际签收规则的订单金额汇总。',

  returnAmount:

    '退款金额：统计所选日期范围内订单匹配到的真实商品退款金额汇总，纯运费补偿不计入。',

  orderCount: '支付单数：统计所选日期范围内有支付时间的已支付订单数，按 P 订单号去重。',
  ...
  qualityReturnCount:
    ...
    '品退单数：统计官方品质负反馈命中且匹配订单，或售后明确商品质量问题的订单数，按 P 单号去重。',

  returnRate: '退款率 = 退款单数 ÷ 支付单数（后端 summary.returnRate，前端不自行计算）。',

  signRate: '签收率 = 签收单数 ÷ 支付单数（后端 summary.signRate，前端不自行计算）。',

  qualityReturnRate:

    '品退率 = 品退单数 ÷ 支付单数。数据来源：官方品质负反馈 + 售后接口交叉印证。',
